...
| Symbol | Purpose |
| --- | --- |
| `Hyphenator` | Splits words using a pattern set; caches results |
| `Hyphenator.fromSource` | Compiles a hyphenator from pattern file text |
| `TexHyphenationPatterns` | A compiled pattern set, shareable between hyphenators |
| `HyphenLineBreaker` | Greedy width-aware line breaking |
| `BreakCandidate` | One break opportunity considered by the breaker |
| `DanglingWords` | An allocation-free probe table of words to keep together |
| `minifyTexPatterns` | Strips a pattern file to its pattern groups |
| `kSoftHyphen` | `U+00AD`, the default `hyphenate` separator |

### `package:flutter_hyphenation`

| Symbol | Purpose |
| --- | --- |
| `HyphenText` | A drop-in `Text` that hyphenates |
| `HyphenScope` | Provides a hyphenator to a subtree |
| `HyphenationRegistry` | App-wide pattern sets keyed by locale |
| `HyphenatorAsset.fromAsset` | Loads and compiles a hyphenator from the bundle |
| `HyphenParagraph` | The render object behind `HyphenText` |
