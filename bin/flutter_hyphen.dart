...
Future<void> main(List<String> arguments) async {
  ...
  try {
    ...
    stderr.writeln('flutter_hyphen: ${error.message}');
  ...
  if (inputPath == null || outputPath == null) {
    stderr.writeln('flutter_hyphen: --input and --output are required.');
    stderr.writeln(_usage);
    exitCode = 2;
    return;
  }
  ...
  if (!input.existsSync()) {
    stderr.writeln('flutter_hyphen: no such file: $inputPath');
    exitCode = 2;
    return;
  }
  ...
  if (layout == null) {
    stderr.writeln('flutter_hyphen: unknown layout: $layoutName');
    stderr.writeln(_usage);
    exitCode = 2;
    return;
  }
  ...
  if (width == null || width <= 0) {
    stderr.writeln('flutter_hyphen: --width must be a positive integer.');
    exitCode = 2;
    return;
  }
  ...
  if (result.patternCount == 0) {
    ...
    stderr.writeln('flutter_hyphen: no \\patterns group found in $inputPath');
    exitCode = 1;
    return;
  }
