...
Future<void> main(List<String> arguments) async {
  ...
  try {
    ...
    stderr.writeln('flutter_hyphenate: ${error.message}');
  ...
  if (inputPath == null || outputPath == null) {
    stderr.writeln('flutter_hyphenate: --input and --output are required.');
    stderr.writeln(_usage);
    exitCode = 2;
    return;
  }
  ...
  if (!input.existsSync()) {
    stderr.writeln('flutter_hyphenate: no such file: $inputPath');
    exitCode = 2;
    return;
  }
  ...
  if (layout == null) {
    stderr.writeln('flutter_hyphenate: unknown layout: $layoutName');
    stderr.writeln(_usage);
    exitCode = 2;
    return;
  }
  ...
  if (width == null || width <= 0) {
    stderr.writeln('flutter_hyphenate: --width must be a positive integer.');
    exitCode = 2;
    return;
  }
  ...
  if (result.patternCount == 0) {
    ...
    stderr.writeln('flutter_hyphenate: no \\patterns group found in $inputPath');
    exitCode = 1;
    return;
  }
  ...
  if (options.containsKey('verbose')) {
    ...
    stdout.writeln(
      'flutter_hyphenate: $inputPath ${before}B -> ${after}B (-$saved%), '
